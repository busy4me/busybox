...
start () {
  ...
  _OPT_5=$6
  if [ -z "$_TASK" ]; then
    screen -dmS frame00$DISPLAY '/opt/busybox/frame00' # default task to run: frame00
    sleep 2
    screen -ls
    return
  fi
...
start-walking () {
	...
	screen_start=$(screen -dmS fb-walking-around$DISPLAY '/opt/busybox/fb/fb-walking-around')
	echo $screen_start
}
...
start-fb-save-my-groups () {
	screen_start=$(screen -dmS fb-save-my-groups$DISPLAY '/opt/busybox/fb/fb-save-my-groups')
	echo $screen_start
}
...
case $1 in
	all|--all|-a)
		...
		stop-mouse-move-around
		stop-walking
		screen_task="/opt/busybox/fb/fb-post COMMENT "$1
	...
	"all"|"fb-post -c all"|"fb-post")
		...
		case $cron in
			*$task*)
			...
			crontab -l | echo "*/5 * * * * export DISPLAY=:0 && /opt/busybox/fb/fb-post -c all" | crontab - | logline
			;;
			*)
...
function CRON () {
  ...
  case $1 in
	disable|--disable)
	  ...
	  crontab -l | sed "/^[^#].*export DISPLAY=$DISPLAY && \/opt\/busybox\/fb\/$job/s/^/#/" | crontab -
	  crontab -l | sed "/^[^#].*export DISPLAY=$DISPLAY && \/opt\/busybox\/busy/s/^/#/" | crontab -
	  crontab -l
    ;;
	enable|--enable)
	...
	--create-table|--create)
	  ...
	  case $tab in
		...
		fb_group__metadata)
			...
			# 3  | admin | /StephenBusybox       | /StephenBusybox               | 2019-05-10 | ok     | 1       | 1     | 0     |
		...
		fb_page__metadata)
			...
			# 2  | admin | /StephenBusybox1   | Stephen Busybox | 2019-05-10 | active | 3       | 1     |
	...
	--update-data|--update|-ud)
		case $tab in
			groups|fb_groups)
				...
				case $3 in
					category|--category|-cat|-c)
						# screen -dmS fb-groups-$3\:$4$DISPLAY '/opt/busybox/fb/fb-groups $3 $4'
						/bin/bash /opt/busybox/fb/fb-groups $3 $4
					;;
					--key|--key-random|--key-table)
						screen_task="/opt/busybox/fb/fb-groups "$3" "$4
                    	# /bin/bash /opt/busybox/fb/fb-groups $3 $4
					;;
					--collect-info)
						screen_task="/opt/busybox/fb/fb-collect-info "$3" "$4
                    	# /bin/bash /opt/busybox/fb/fb-collect-info $3 $4
					;;
					--save-my-groups)
						screen_task="/opt/busybox/fb/fb-save-my-groups"
						# /bin/bash /opt/busybox/fb/fb-save-my-groups
					;;
					'')
			...
			fb_pages)
				...
				case $3 in
					--save-my-pages)
						/bin/bash /opt/busybox/fb/fb-save-my-pages
					;;
					*)
						echo -e "\e[41m wrong --method argument ... \e[0m\e[95m $3 $4 \e[33m\e[0m"
					;;
			...
			fb_friends)
				...
				case $3 in
					--save-my-friends)
						/bin/bash /opt/busybox/fb/fb-save-my-friends
					;;
					*)
						echo -e "\e[41m wrong --method argument ... \e[0m\e[95m $3 $4 \e[33m\e[0m"
					;;
...
if [ $USER = "root" ]; then
 ...
 \e[92m[info]\e[0m login as busybox ...\e[0m ... \n usage: busy --help"
 su busybox
fi
