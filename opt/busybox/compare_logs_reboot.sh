...
# if same that means nothing happening and reboot, and write 0 to file /tmp/busybox-reboot
...
logcompare() {
  ...
  echo -e "\e[32m Remove old busybox_tmp.log... \e[0m"
  rm /tmp/${PROJECT}_tmp.log
  echo -e "\e[32m Copy new busybox.log... \e[0m"
  ...
  if [ -f /tmp/${PROJECT}-reboot ]; then
	...
	echo -e "\e[32m No... \e0m"
	echo "0" > /tmp/${PROJECT}-reboot | logline_cron
	chown busybox:busybox /tmp/busybox-reboot
  fi
}
